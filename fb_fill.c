...
int main(int argc, char **argv)
{
  ...
  unsigned int count, start, end, i;
  struct timeval tv;
  ...
  printf("Fill frame buffer test bench\n");
  printf("Benchmarking fullscreen fill: %.2f MPixels/second\n",
         count * (double)width * height / ((end - start) * 1000.));
  printf("(Theoretical raw write bandwidth upper bound)\n");
