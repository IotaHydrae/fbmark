...
int main(int argc, char **argv)
{
  ...
  /* Classic Julia set constant: c = -0.7 + 0.27i */
  cr = -0.7f;
  ci = 0.27f;
  ...
  while (iters <= 48) {
    for (i = 0; i < height; i++) {
      for (j = 0; j < width; j++) {
        /* Map pixel to complex plane: real range [-1.5, 1.5], imag range [-1.5, 1.5] */
        x = 3.f * j / width - 1.5f;
        y = 1.5f - 3.f * i / height;
        c = 0;
  ...
  printf("Julia frame buffer test bench\n");
