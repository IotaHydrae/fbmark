...
int main(int argc, char **argv)
{
  ...
  unsigned int count, start, end, dy, i;
  struct timeval tv;
  ...
  do {
    ...
    /* scroll up by dy rows */
    memmove(buffer + posy * row_size + posx * pixel_size,
            buffer + (posy + dy) * row_size + posx * pixel_size,
            (height - dy) * row_size);

    /* fill new bottom rows */
    for (i = height - dy; i < height; i++)
      memset(buffer + (posy + i) * row_size + posx * pixel_size,
             rand() % 256, width * pixel_size);
  ...
  printf("Scroll frame buffer test bench\n");
  printf("Benchmarking scroll: %d scrolls in %.2f seconds (%.2f scrolls/second)\n",
         count, (end - start) / 1000., count * 1000. / (end - start));
  printf("Effective bandwidth: %.2f MPixels/second\n",
